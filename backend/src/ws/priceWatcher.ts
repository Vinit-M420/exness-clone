...
export async function priceWatcher(){
    ...
    const openOrders = await db
        .select({ id: orders.id, side: orders.side, symbol: orders.symbol, triggerPrice: orders.triggerPrice })
        .from(orders)
        .where(and(eq(orders.status, "pending"), eq( orders.orderType, "limit")))
